...
static void update_split_count(pte_t *kpte, pgprot_t old_prot, pgprot_t new_prot, enum pg_level level)
{
...
static void merge_mapping(pte_t *parent, pte_t *kpte, pgprot_t new_prot)
{
	...
	ptep = first_entry;
	for (int i = 0; i < PTRS_PER_PTE; i++, ptep++) {
		BUG_ON(!standard_pgprot(pte_pgprot(*ptep)));
	}
				     ...
				     struct cpa_data *cpa)
{
	...
	if (level == PG_LEVEL_2M) {
		update_split_count((pte_t *)cpa->pud, old_prot, new_prot, PG_LEVEL_2M);
		   ...
		   struct page *base)
{
	...
	for (i = 0; i < PTRS_PER_PTE; i++, pfn += pfninc, lpaddr += lpinc) {
		...
		actual_prot = pte_pgprot(*(pbase + i));
		if (!standard_pgprot(actual_prot)) {
			base->split_count++;
		}
	}
	...
	if (level == PG_LEVEL_2M) {
		...
		update_split_count((pte_t *)cpa->pud, ref_prot, __pgprot(_KERNPG_TABLE), level);
	}
...
repeat:
	...
	if (level == PG_LEVEL_4K) {
		...
		update_split_count((pte_t *)cpa->pmd, old_prot, new_prot, PG_LEVEL_4K);
