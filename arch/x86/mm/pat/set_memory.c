...
{
	prot = __pgprot(pgprot_val(prot) & ~(_PAGE_PSE | _PAGE_PAT));
...
static void update_foo(pte_t *kpte, pgprot_t old_prot, pgprot_t new_prot, enum pg_level level)
{
	...
	// printk("merged-level = %s, parent's split_count = %d\n", name, page->split_count);
...
static void merge_mapping(pte_t *parent, pte_t *kpte, pgprot_t new_prot)
{
	...
	new_prot = __pgprot(pgprot_val(pgprot_4k_2_large(new_prot)) | _PAGE_PSE);
	set_pte_atomic(kpte, pfn_pte(pte_pfn(*first_entry), new_prot));

	/* free page table */
	page->split = 0;
	page->split_count = 0;
	pte_free(&init_mm, page);

	/* when PTEs are merged into PMD, split_count of PUD should be udpated */
	if (parent) {
		page = phys_to_virt(pte_pfn(*parent) << PAGE_SHIFT);
