...
static bool should_merge_mapping(pte_t *kpte, pte_t *new_pte,
				 unsigned n_ptrs, unsigned pfn_diff)
{
	int i;

	/* PMD cannot have RO mapping */
	if (!(pte_flags(*new_pte) & _PAGE_RW))
		return false;

	for (i = 0; i < n_ptrs; i++, kpte++) {
		if (pte_none(*kpte) || pte_flags(*kpte) != pte_flags(*new_pte))
			return false;
